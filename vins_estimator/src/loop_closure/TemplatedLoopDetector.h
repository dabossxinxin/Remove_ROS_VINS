...
protected:
	 ...
	 */
	void computeIslands(QueryResults &q, std::vector<tIsland> &islands) const;
	...
	void reduceInputToOutput(std::vector<cv::Point2f> v_in, std::vector<cv::Point2f> &v_out, std::vector<uchar> status);
	bool checkFoundamental(std::vector<cv::Point2f> cur_input_pts,
		std::vector<cv::Point2f> old_input_pts,
		std::vector<cv::Point2f> &cur_output_pts,
		std::vector<cv::Point2f> &old_output_pts);
	/**
...
(QueryResults &q, std::vector<tIsland> &islands) const
{
	islands.clear();
...
// --------------------------------------------------------------------------
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::reduceInputToOutput(std::vector<cv::Point2f> v_in, std::vector<cv::Point2f> &v_out, std::vector<uchar> status)
{
	v_out.clear();
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::checkFoundamental(std::vector<cv::Point2f> cur_input_pts,
	std::vector<cv::Point2f> old_input_pts,
	std::vector<cv::Point2f> &cur_output_pts,
	std::vector<cv::Point2f> &old_output_pts)
{
	if (cur_input_pts.size() >= 8)
	{
		...
		std::vector<cv::Point2f> un_cur_pts(cur_input_pts.size()), un_old_pts(cur_input_pts.size());
